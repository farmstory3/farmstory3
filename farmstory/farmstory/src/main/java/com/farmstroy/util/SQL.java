...
public class SQL {
	...
	public static final String SELECT_MARKETPRODUCTS_TEST = "SELECT PF.`path`, PF.filename, P.prod_idx, P.prod_name, P.`type`, P.price, P.stock, P.regDate, P.point, P.discount "
			+ "FROM product AS P JOIN product_file AS PF ON P.prod_idx = PF.prod_idx WHERE PF.`type` = 1 ";
	...
	public static final String SELECT_CART_ITEM_SUM_BY_UID = "select sum(quantity) as total_quantity, sum(price) as total_price, sum(discount) as total_discount, sum(`point`) as total_point, sum(price/100) as total_savepoint, sum(`sum`) as total_sum FROM cart_item where uid = ?";
	public static final String SELECT_CART_BY_UID = "SELECT * FROM cart WHERE uid = ?";
	public static final String SELECT_CART_ITEM_BY_UID = "SELECT * FROM cart_item WHERE uid = ?";
	...
	public static final String DELETE_CART_ITEMS = "delete from cart_item where cart_item_idx = ?";
}
