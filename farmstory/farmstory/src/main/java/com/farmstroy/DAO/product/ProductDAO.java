...
public class ProductDAO extends DBHelper{
	...
	public List<ProductListDTO> selectMarketProductList(String type) {
		...
		try {
			...
			while(rs.next()) {
				...
				productList.setStock(rs.getInt("stock"));
				productList.setRegDate(rs.getString("regdate"));
				productList.setPoint(rs.getBigDecimal("point"));
				productList.setDiscount(rs.getInt("discount"));
				productLists.add(productList);
			}
		} catch (Exception e) {
	...
	public int SELECT_MARKETPRODUCTS_COUNT() {
		...
		try {
			...
			rs = stmt.executeQuery(SQL.SELECT_MARKETPRODUCTS_COUNT);
			System.out.println("rs눈 "+ rs);
			if(rs.next()) {
				count = rs.getInt(1);
			}
		} catch (Exception e) {
	...
	public List<String> SELECT_MARKETPRODUCTS_TYPE() {
		...
		try {
			conn =  getConnection();
			stmt = conn.createStatement();
			rs = stmt.executeQuery(SQL.SELECT_MARKETPRODUCTS_TYPE);
			while(rs.next()) {
	...
	public ProductDTO selectProduct(String prod_idx) {
		...
		try {
			...
			product.setProd_file(files);
			closeAll();
		}
		return product;
	}

	public int select_marketproducts_count_by_type(String type) {
		int count = 0;
		try {
			conn =  getConnection();
			psmt = conn.prepareStatement(SQL.SELECT_MARKETPRODUCTS_COUNT_BY_TYPE);
			psmt.setString(1, type);
			rs = psmt.executeQuery();
			System.out.println("rs눈 "+ rs);
			if(rs.next()) {
	...
	public List<ProductListDTO> selectMarketProductListTest() {
List<ProductListDTO> productLists = new ArrayList<ProductListDTO>();
		
		try {
			conn = getConnection();
			psmt = conn.prepareStatement(SQL.SELECT_MARKETPRODUCTS_TEST);
			rs = psmt.executeQuery();
			while(rs.next()) {
				ProductListDTO productList = new ProductListDTO();
				productList.setPath(rs.getString("path"));
